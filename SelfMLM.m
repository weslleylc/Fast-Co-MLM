function SelfMLM(numberRepetitions,numberRepetitionsForCotraining,normalization,select,ParamLabel,param,Directory,DataName)
    DataName
    validation=1;
    bias = 0;
    lambda = 0; %10e-10;
                ...
                for v=1:validation,
                    ...
                    for alpha=param,                        
                        ...
                        for k=1:numberRepetitionsForCotraining                    
                            %for each view
                            %%basic MLM Training
                            data.x = dataset.xtrain{i}(L,1:end-1);
                            data.y = Y_train(L,:);      
                            ...
                            UnlabeledData.x = dataset.xtrain{i}(U,1:end-1); 
                            UnlabeledData.y = Y_train(U,:);   
                            %%R.D(view), model dor each view 
                            [R.D, ~] = test_MLM(Model, UnlabeledData, type);
                            Data.x = originalData.x; 
                            Data.y = originalData.y;                                
                            [Yh, ~] = test_MLM(Model, Data, type);
                            ...
                            [bestIndex,badIndex] = bestSeltMLM(R,1);
