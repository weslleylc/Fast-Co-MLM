function [MODEL]=SelfNNMLM(objParam)
    ...
    if selectMethod~=0
        dataName=strcat(dataName,'_Random')
    end
    ...
    for labeled=paramLabel,
                ...
                for v=1:objParam.validation,
                    ...
                    for alpha=objParam.paramPoints,                        
                        ...
                        for k=1:numberRepetitionsForCotraining                    
                            %for each view
                            %%basic MLM sel-Training
                            data.x = objParam.dataSet.xtrain{i}(L,1:end-1);
                            [Lin Col]=size(data.x);
                            data.y = Y_train(L,:);      
                            if k==1
                                ...
                                refPoints.x = data.x(Points, :);
                                refPoints.y = data.y(Points, :);
                            end
                            ...
                            if (k==1)
                                ...
                                [ Model,P ] = Multi_RLS_Online(Model,P,newDx(Lin-new+1:end,:),newDy(Lin-new+1:end,:));
                            end

                            UnlabeledData.x = objParam.dataSet.xtrain{i}(U,1:end-1); 
                            UnlabeledData.y = Y_train(U,:);   
                            %%R.D(view), objParam dor each view 
                            [R.D,R.C]=test_nnMLM(Model,UnlabeledData,refPoints);                             
                            R.Ref=refPoints;

                            Data.x = originalData.x; 
                            Data.y = originalData.y;                                
                            [~,Yh]=test_nnMLM(Model,Data,refPoints);

                            for j = 1: length(Data.y),
                            [~, indexVal(j)] = max(Yh(j,:));
                            [~, targetVal(j)] = max(Data.y(j, :));
                            end  
                            R.Rate = length(find(indexVal == targetVal))/length(Data.y);
                            
                            %%%the best exemple per class for each classifier
                            [bestIndex,badIndex, classes] = best_self(R,1);
        ...
        if numberRepetitionsForCotraining>0
            s1 = strcat(dataName,'_SelfMLM_Online_results_');    
            s2 = num2str(labeled);
